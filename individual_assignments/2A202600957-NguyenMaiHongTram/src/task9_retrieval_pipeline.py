...
def retrieve(
    ...
    """
    # Step 1: Chạy semantic + lexical (lấy dư 2x để có đủ candidates cho rerank)
    try:
        dense_results = semantic_search(query, top_k=top_k * 2)
    except Exception as e:
        print(f"  ⚠ semantic_search lỗi ({e}) — bỏ qua dense retrieval")
        dense_results = []

    try:
        sparse_results = lexical_search(query, top_k=top_k * 2)
    except Exception as e:
        print(f"  ⚠ lexical_search lỗi ({e}) — bỏ qua sparse retrieval")
        sparse_results = []
    ...
    if use_reranking and merged:
        try:
            ...
            print(f"  ⚠ rerank lỗi ({e}) — dùng kết quả merged chưa rerank")
